...
def load_precreated_vgg_nn_and_mean_img():
    ...
    Return:  nolearn NeuralNet instance, mean_image array
    '''
...
def featurize_1000images_pkl(path_prefix, image_urls, start_index, step_size, output_layer, mean_image, label, label_val):
    '''
    *******************************************************************
    **** This code if for featurizing images using lasagne layers. ****
    This code is SLOW, but works fine with smaller datasets of images.
    Better to use function:  parallel_featurize_1000images_pkl()
    *******************************************************************
    Input: path to images on s3, list of image urls (location on aws s3),
           starting index (only 1000 per call), lasagne output_layer,
           mean_image, imageset label, imageset label_val (int)
...
def parallel_featurize_1000images_pkl(path_prefix, image_urls, start_index, step_size, vgg_nn, mean_image,
    ...
    '''
    *** This function is optimized as a target for multiprocessing.Pool.map ***
    Input: path to s3 collection, list of image urls (location on aws s3),
           starting index for counting through url list, step_size,
           mean_image array, imageset label (string), imageset label_val (int)
    ...
    # make sure stopping point is not past the end of the list
    if len(image_urls) < start_index+step_size:
        end_index = len(image_urls)
    else:
        end_index = step_size + start_index
    ...
    # If an error happens reading the image in prep_image_parallel,
    #   it will return an image of size 0.  Remove these list elements.
    img_list = [img for img in img_list if img.size > 0]

    # vectorize image list
    image_vectors = np.concatenate(img_list,axis=0)

    # Use the neural net to featurize the images
    featurized_images = vgg_nn.predict_proba(image_vectors)

    # label and save the features to disk and to s3 storage
...
def featurize_image(full_path_filename, output_layer, mean_image):
    '''
    *** This function is depreciated.   Requries use of lasagne layers.   ***
    *** Currently using nolearn.  It's much, much faster. ***

    Input: full path to the location of the file,
        lasagne output_layer, mean_image array of the vgg training data

    Process the image (vectorize, resize and crop).
    Run image through through the network.
    Output:  features array with shape: (4096,1)
    '''
